...
namespace app\modules\zhengwen\controllers;

use app\modules\zhengwen\models\PrizePool;
use Yii;
use app\modules\zhengwen\BL\StatusTransfer;
use yii\helpers\ArrayHelper;
use yii\web\Controller;
use yii\helpers\Url;
use app\modules\zhengwen\BL\Lottery;
use app\modules\zhengwen\models\WinForm;
...
//        exit;
        if (Yii::$app->userzhengwen->isGuest) {
...
//        }
    ...
    public function actionPost()
    {
        ...
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
            $userExtra = Yii::$app->userzhengwen->identity->extra;
            Yii::$app->userzhengwen->identity->extra = !empty($userExtra) ?
                json_encode(ArrayHelper::merge(is_array(json_decode($userExtra, true)) ? json_decode($userExtra, true) : [Yii::$app->userzhengwen->identity->d1_status == 20 ? 1 : 2 => $userExtra],
                    [StatusTransfer::getOrdinal() => $model->extra])) :
                json_encode([StatusTransfer::getOrdinal() => $model->extra]);

            if (Yii::$app->userzhengwen->identity->save() !== false) {
                return 1;
            }
        }
        return 0;
