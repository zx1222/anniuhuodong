...
use Yii;
use yii\base\Object;
use app\modules\ejiaoyuandan\models\Config;
...
// 随机更新
                    ...
                    $lukeyModel->openid = Yii::$app->userejiaoyuandan->getId();
                    $lukeyModel->ip = $userIP;


                    if (PrizePool::updateAll(['status' => 10, 'openid' => Yii::$app->userejiaoyuandan->getId(), 'ip' => $userIP], ['status' => 0, 'pid' => $lukeyModel->pid]) !== false) {
                        $res = $prize_arr[$rid];
                    }
                }
