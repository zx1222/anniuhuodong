...
use Yii;
use yii\base\Object;
use app\modules\wuhan\models\Config;
...
class Lottery extends Object
{
    ...
    static public function run()
    {
        ...
        $_prize_arr = Yii::$app->cache->get('whconfig');
        if ($_prize_arr == false) {
            $_prize_arr = Config::find()->asArray()->all();
            Yii::$app->cache->set('whconfig', $_prize_arr, 3600);
        }
...
// 随机更新
                ...
                $lukeyModel->status = 1;
                $lukeyModel->openid = Yii::$app->userxipuhui->getId();
                $lukeyModel->ip = $_SERVER["REMOTE_ADDR"];


                if (PrizePool::updateAll(['status' => 1, 'openid' => Yii::$app->userxipuhui->getId(), 'ip' => $_SERVER["REMOTE_ADDR"]], ['status' => 0, 'pid' => $lukeyModel->pid]) !== false) {
                    $res = $prize_arr[$rid];
                }
            }
        }
// 更新状态
    ...
    {
        $model = Yii::$app->cache->get('whbad');

        if ($model == false) {
            $model = Config::findOne($id);
            Yii::$app->cache->set('whbad', $model, 3600);
        }
