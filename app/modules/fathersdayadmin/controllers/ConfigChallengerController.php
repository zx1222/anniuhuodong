 ...
 */
class ConfigChallengerController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionInitPool()
    {
        ...
        $prizeType = ConfigChallenger::find()->all();
        foreach ($prizeType as $k => $v) {

            if ($v->praisenumber > 0) {
                for ($i = 0; $i < $v->praisenumber; $i++) {
                    $model = new PrizePoolChallenger();
                    $model->aid = $v->id;
                    $model->save();
                    unset($model);
                }
...
//        $this->findModel($id)->delete();
    ...
    {
        if (($model = ConfigChallenger::findOne($id)) !== null) {
            return $model;
        } else {
