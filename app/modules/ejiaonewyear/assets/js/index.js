...
$(function () {
    ...
    //机会用尽标识
    var chance_end=$('#is_chance').val();
    if (chance_end == 1) {
        $('.mask-win .btn-close,.mask-win .panel-noChance').removeClass('none');
        $('.mask-win .btn-toDraw,.mask-win .panel-noWin').addClass('none');
    }
    else {
        $('.mask-win .btn-close,.mask-win .panel-noChance').addClass('none');
        $('.mask-win .btn-toDraw,.mask-win .panel-win').removeClass('none');
    }
...
//未中奖,机会用尽,提交成功关闭 未过关点击否
    $('.mask-noPrize .btn-close,.mask-noChance .btn-close,.mask-submitok .btn-close,.btn-no,.mask-win .btn-close').on('click', function () {
