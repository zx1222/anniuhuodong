...

{
    ...
    {
        $userInfo = Yii::$app->useranniuyuandan->getIdentity()->toArray();
    ...
    static public function markRenew()
    {
        ...
        if ($toDayStatus < self::FORBIDDEN_STATUS && $toDayStatus % 10 == 0) {
            ...
            Yii::$app->useranniuyuandan->identity->$toDayStatusField = $currentStatus;
            Yii::$app->useranniuyuandan->identity->save();
        }
    ...
    static public function markPause($prize = false)
    {
        ...
        if ($prize != false && $prize['praisenumber'] > 0) {
            Yii::$app->useranniuyuandan->identity->$toDayPidField = $prize['id'];
        }
        Yii::$app->useranniuyuandan->identity->save();
