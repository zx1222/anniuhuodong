...
namespace app\modules\anniuyuandan\BL;

use app\modules\anniuyuandan\models\WeixinUser;
use Yii;
use yii\base\Object;
use app\modules\anniuyuandan\models\Config;
...
// 随机更新
                    ...
                    $lukeyModel->openid = Yii::$app->useranniuyuandan->getId();
                    $lukeyModel->ip = $userIP;


                    if (PrizePool::updateAll(['status' => 10, 'openid' => Yii::$app->useranniuyuandan->getId(), 'ip' => $userIP], ['status' => 0, 'pid' => $lukeyModel->pid]) !== false) {
                        $res = $prize_arr[$rid];
                    }
                }
