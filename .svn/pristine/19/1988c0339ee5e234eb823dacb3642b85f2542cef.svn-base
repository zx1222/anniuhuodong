...

{
    ...
    {
        $userInfo = Yii::$app->userchongyang->getIdentity()->toArray();
    ...
    static public function markRenew()
    {
        ...
        if ($toDayStatus < self::FORBIDDEN_STATUS && $toDayStatus % 10 == 0) {
            ...
            Yii::$app->userchongyang->identity->$toDayStatusField = $currentStatus;
            Yii::$app->userchongyang->identity->save();
        }
    ...
    static public function markPause($prize = false)
    {
        ...
        if ($prize != false && $prize['praisenumber'] > 0) {
            Yii::$app->userchongyang->identity->$toDayPidField = $prize['id'];
        }
        Yii::$app->userchongyang->identity->save();
