...
namespace app\modules\xipuhui\BL;

use Yii;
use yii\base\Object;
use app\modules\xipuhui\models\Config;
use app\modules\xipuhui\models\PrizePool;
...
// 更新状态
    ...
    {
        $model = Yii::$app->cache->get('xphbad');

        if ($model == false) {
            $model = Config::findOne($id);
            Yii::$app->cache->set('xphbad', $model, 3600);
        }
