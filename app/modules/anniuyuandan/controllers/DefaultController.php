...
namespace app\modules\anniuyuandan\controllers;

use app\modules\anniuyuandan\models\PrizePool;
use Yii;
use app\modules\anniuyuandan\BL\StatusTransfer;
use yii\helpers\ArrayHelper;
use yii\web\Controller;
use yii\helpers\Url;
use app\modules\anniuyuandan\BL\Lottery;
use app\modules\anniuyuandan\models\WinForm;
...
//        exit;
        if (Yii::$app->useranniuyuandan->isGuest) {
...
//        }
    ...
    public function actionPost()
    {
        ...
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
            $userExtra = Yii::$app->useranniuyuandan->identity->extra;
            Yii::$app->useranniuyuandan->identity->extra = !empty($userExtra) ?
                json_encode(ArrayHelper::merge(is_array(json_decode($userExtra, true)) ? json_decode($userExtra, true) : [Yii::$app->useranniuyuandan->identity->d1_status == 20 ? 1 : 2 => $userExtra],
                    [StatusTransfer::getOrdinal() => $model->extra])) :
                json_encode([StatusTransfer::getOrdinal() => $model->extra]);

            if (Yii::$app->useranniuyuandan->identity->save() !== false) {
                return 1;
            }
        }
        return 0;
