...
use app\modules\ejiaoduilian\models\WeixinUser;
use Yii;
use yii\base\Object;
...
// 随机更新
                    ...
                    $lukeyModel->status = 1;
                    $lukeyModel->openid = Yii::$app->userejiaoduilian->getId();
                    $lukeyModel->ip = $userIP;


                    if (PrizePool::updateAll(['status' => 1, 'openid' => Yii::$app->userejiaoduilian->getId(), 'ip' => $userIP], ['status' => 0, 'pid' => $lukeyModel->pid]) !== false) {
                        $res = $prize_arr[$rid];
                    }
                }
